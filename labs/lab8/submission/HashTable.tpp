...
HashTable<K,V>::HashTable(unsigned int table_size, unsigned int(*hash_func)(const K&), CollisionResolutionAlgo algo, unsigned int(*hash_func2)(const K&))
  : table_size(table_size), table(new Entry<K,V>[table_size]), hash_func(hash_func), hash_func2(hash_func2), algo(algo){
}
// TODO #1: --- END ---
...
unsigned int HashTable<K,V>::offset(int i, const K& key){
  if(algo==LINEAR)
    ...
    return i*HashTable<K,V>::hash_func2(key);
}
// TODO #3: --- END ---
...
unsigned int HashTable<K,V>::remove(const K& key) {
  ...
  while(i < table_size * 2) { // Assume the number of probes should be less than 2*table_size
    ...
    // TODO #5: --- BEGIN ---     
    if(table[j].key==key&&table[j].status==ACTIVE){
...
unsigned int HashTable<K,V>::find(const K& key, V*& v) {
  ...
  while(i < table_size * 2) { // Assume the number of probes should be less than 2*table_size
    ...
    else if(table[j].key==key&&table[j].status==DELETE){
      v=nullptr;
      return i;
    }
    // TODO #6: --- END ---
