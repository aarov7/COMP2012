...
BinarySearchTree<T1, T2>::BinarySearchTree(const BinarySearchTree<T1, T2>& other_tree):
    name(other_tree.name), score(other_tree.score), left_sub_tree(nullptr), right_sub_tree(nullptr) {
...
bool BinarySearchTree<T1, T2>::findByScorePair(const T1 &x, const T2 &y) const {
    ...
    if(left_sub_tree!=nullptr&&(score.baseScore>x||(score.baseScore==x&&score.bonusScore>y))){
        return left_sub_tree->findByScorePair(x,y);
    }
    if(right_sub_tree!=nullptr&&(score.baseScore<x||(score.baseScore==x&&score.bonusScore<y))){
...
bool BinarySearchTree<T1, T2>::findByName(const string& name) const {
    ...
    //just simple dfs because names are not sorted in any way
    if(this->name==name){
        printPoint();
        return true;
    }
    ...
    return left_sub_tree->findByName(name)||right_sub_tree->findByName(name);
}
...
void BinarySearchTree<T1, T2>::insert(const string& name, const T1 &x, const T2 &y) {
    ...
    //the tree you are inserting in will always have at least one node
    
    if(score.baseScore>x||(score.baseScore==x&&score.bonusScore>y)){
        //insert to left
        if(left_sub_tree)
            left_sub_tree->insert(name,x,y);
        else{
            BinarySearchTree<T1, T2>* node=new BinarySearchTree(name,x,y);
            left_sub_tree=node;
        }
    }
    if(score.baseScore<x||(score.baseScore==x&&score.bonusScore<y)){
