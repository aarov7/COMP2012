...
template <class T, int MAX_SIZE, bool ASCENDING>
std::ostream& operator<<(std::ostream& os, const SortedArray<T, MAX_SIZE, ASCENDING>& a) {
    if (!a.empty()) {
        int i = 0;
        os << "{";
        for (; i < (a.size() - 1); ++i) 
...
template<typename T, int MAX_SIZE, bool ascending>
SortedArray<T, MAX_SIZE, ascending> operator+(const SortedArray<T, MAX_SIZE, ascending>& a, const SortedArray<T, MAX_SIZE, ascending>& b){
    SortedArray<T, MAX_SIZE, ascending> c(a);
    int bInd=0;
    while(bInd<b.size()&&c.size()<MAX_SIZE){
        c.insert(b[bInd++]);
    }
    return c;
}
